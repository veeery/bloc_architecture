...
class FriendListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<FriendBloc>(
      create: (context) => FriendBloc()..add(GetAllFriend()),
      child: BaseWidgets(
          onRefresh: () async {
            ...
            BlocBuilder<FriendBloc, FriendState>(
              builder: (context, state) {
                if (state is LoadingFriendState) {
                  ...
                  return AppEmpty(message: state.message, onTap: () => context.read<FriendBloc>().add(GetAllFriend()));
                } else if (state is LoadFriendState) {
                  final userList = state.userList;
                  if (userList.isEmpty) {
                    return AppEmpty(
                        message: "Klik to Refresh", onTap: () => context.read<FriendBloc>().add(GetAllFriend()));
                  } else {
